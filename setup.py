...
def program_in_path(program):
    return exec(f"which {program}", capture=True).returncode == 0
...
def maybe_append_path(path):
    with open("~/.zshcustom") as zshcustom:
...
def install(name):
    ...
    try:
        if os == OS.Darwin and arch == Architecture.arm_64 and hasattr(installer, 'osx_silicon'):
